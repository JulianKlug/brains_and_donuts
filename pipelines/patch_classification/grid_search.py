...
from topological_3D_patch_classification import evaluate_topological_3D_patch_classification
...
## Import data
data_path = '/home/klug/working_data/angio_CT/withAngio_all_2016_2017.npz'
save_dir = '/home/klug/output/bnd/stroke_patch_classification_grid_search'
result_file = '3D_patch_grid_search.csv'
n_subjects = None
n_threads = 50
test_set_seed = 42
...
for params in tqdm(list(ParameterGrid(params_search_space))):
        ...
        experiment_uid = create_experiment_name(params, mode='hex', save_dir=save_dir)
        ...
        try:
            evaluation_output = evaluate_topological_3D_patch_classification(
                X_train, y_train, mask_train, save_dir, experiment_uid,
                features=params['features'],
                amplitude_metric=params['amplitude_metric'],
                processing_filter=params['processing_filter'], processing_scale=processing_scale,
                processing_scaler_metric=params['processing_scaler_metric'],
                homology_dimensions=params['homology_dimensions'], inverse_input=params['inverse_input'],
                width=params['width'], undersampling=params['undersampling'],
                n_subjects=n_subjects, n_threads=n_threads, subsampling_factor=params['subsampling_factor'],
                verbose=True,
                save_input_features=False, save_output=False, model=params['model']
            )

            train_acc, valid_acc, train_roc_auc, valid_roc_auc, n_features, subimage_creation_timing, feature_creation_timing, feature_classification_and_prediction_time = evaluation_output

            experiment_results = pd.DataFrame([[]])
            experiment_results['experiment_name'] = experiment_name
            experiment_results['experiment_uid'] = experiment_uid
            experiment_results['train_accuracy'] = train_acc
            experiment_results['validation_accuracy'] = valid_acc
            experiment_results['train_roc_auc'] = train_roc_auc
            experiment_results['validation_roc_auc'] = valid_roc_auc
            experiment_results['n_features'] = n_features
            experiment_results['subimage_creation_timing'] = subimage_creation_timing
